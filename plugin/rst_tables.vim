...
def pad_fields(row, widths):
    ...
    new_row = []
    for i in range(len(row)):
        unicode_len = str_width(row[i].decode('utf-8'))
...
def wrap_text(text, width):
    ...
    while len(text) > 0:
        ...
        while str_width(guess) > width:
            ...
            guess = textwrap.wrap(text, w)[0]
        lines.append(guess.encode('utf-8'))
...
if !exists("no_plugin_maps") && !exists("no_rst_table_maps")
    if !hasmapto('ReformatTable(')
        noremap <silent> <leader><leader>c :python ReformatTable()<CR>
    endif
    if !hasmapto('ReflowTable(')
        noremap <silent> <leader><leader>f :python ReflowTable()<CR>
    endif
endif
